      ...
      <thead>
        <tr >
          {labels && labels.map((label: string, index: number) => (
            <th style={{ textAlign: "center", backgroundColor: "#a6c732" }} key={index} scope="col" hidden={['Id', 'Imágen', 'Imágenes', 'Cargar detalles', 'Tiene sucursales', 'Promociones'].includes(label)}>
              <b>{label}</b>
            </th>
          ))}
          {!sinEditar &&
            <th style={{ textAlign: "center", backgroundColor: "#a6c732" }} scope="col">
              <b>Modificar</b>
            </th>}
          <th style={{ textAlign: "center", backgroundColor: "#a6c732" }} scope="col">
            <b>Eliminar</b>
            ...
            {keys.map((key, index) => (
              <td key={index} hidden={['id', 'imagen', 'imagenes', 'articuloManufacturadoDetalles', 'sucursales', 'promocionDetalles', 'type', 'promociones'].includes(String(key))}>
                {!['esParaElaborar', 'casaMatriz'].includes(String(key))
                  ...
                  : <a className={entidadI[key] ? "btn btn-success" : "btn btn-dark"} 
                  style={{
                    width: '100px',
                    marginBottom: '10px',
                  ...
                  onClick={() => { cambiarBooleano(entidadI.id, String(key)) }}>{entidadI[key] ? "Sí" : "No"}</a>
                }
              </td>
            ))}
            {!sinEditar && (
              <td >
                <Button
                  style={{ letterSpacing: "1px", fontWeight: "bold", backgroundColor: "#a6c732", color: "#FFFFBF" }}
                  className="custom-button"
                  variant="contained"
                  color="primary" onClick={() => { handleOpenModal(entidadI.id) }}>Modificar
                </Button>
              </td>
            )}
            <td >
              <Button
                style={{ letterSpacing: "1px", fontWeight: "bold", backgroundColor: "#e05151", color: "#FFFFBF" }}
                className="custom-button"
                variant="contained"
                color="primary" onClick={() => deleteEntidad(entidadI.id)}>Eliminar
              </Button>
            </td>
          </tr>
        ))}
